...
void DMA::fill_buffers(size_t rows, size_t cols, uint16_t samples[]) {
  for (size_t a = 0; a < rows; a++) {
    uint32_t* buffer = buffers[a];
    for (size_t b = 0; b < cols; b++ ) {
      buffer[b] = samples[(a * cols) + b];
    }
  }
};

void DMA::create_buffers(size_t rows, size_t cols, uint16_t samples[]) {
  ...
  fill_buffers(rows, cols, samples);
};
...
uintptr_t DMA::init(size_t rows, size_t cols, uint16_t samples[]) {
  create_buffers(rows, cols, samples);
